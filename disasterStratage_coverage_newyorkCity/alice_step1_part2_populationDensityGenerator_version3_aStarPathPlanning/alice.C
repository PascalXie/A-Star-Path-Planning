...
//double ObstacleValue_ = 100000.0015; // fake
...
void ComputePath(string computeName, double x, double y)
{
	...
	AStarNode *n_start = new AStarNode("start",2,0);
	pp.SetStartNode(n_start);
	pp.SetStartNode(n_start);
	pp.SetStartNode(n_start);

	AStarNode *n_end = new AStarNode("end1",25,25);
	pp.SetEndNode(n_end);
...
bool Tool_DistributionObstacles()
{
	...
	for(int j=0;j<binSizes[1];j++)
	{
		...
		//hObstacles_->Fill(x,y,costFunctionValue[0]);
		hObstacles_->Fill(x,y,isObstacle);
	}
